...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 4:
        print "usage:%s <server_host> <server_port> <req_file>\n" % sys.argv[0]
        sys.exit(1)
    host = sys.argv[1]
    port = int(sys.argv[2])
    #req_str = sys.argv[3]
    req_str = ReadReqFromFile(sys.argv[3])
    print "send:%u:%s" % (len(req_str), req_str)
    beg = time.time()

    log_id = random.randint(0, 99999999)
    resp_str = reqbinserver_cluster([{"host":host, "port":port}], 0, req_str, log_id, 10000000)
    #import simplejson
    #import zlib
    #try:
