...
class WooDbInterface : public DbInterface{
	...
	public :
		...
		// 2015.01.15 add by fengyoung(fengyang3@staff.weibo.com) 	
		int ssget_r(string& strResp, const char* sReq, const uint32_t n_key = 0)
		{
			initialize();
			char* p_result = NULL; 
			this->db_info_struct_.select_type_ = 2;//保证hash化
			if(get_woo_result(n_key, sReq, p_result) < 0)
				return -1;
			if(p_result == NULL)
				return -1; 
			strResp = p_result; 
		...
		// 2015.01.15 add by fengyoung(fengyang3@staff.weibo.com) 
		int ssmget_ri(vector<string>& vtrResp, vector<string>& vtrReq, const bool bNeedClear = true)
		{
			if(bNeedClear)
				vtrResp.clear(); 
			...
			uint32_t pot = rand() % ip_num_;	
			for(uint32_t index = 0; index < key_num; index++)
				n_keys[index] = (pot + index) % ip_num_;
...
//					continue;
				...
				int ret = binary_client_send(cli[total_i], vtrReq[total_i].c_str(), vtrReq[total_i].length() + 1,
						log_id, true);
		...
		// 2015.01.19 add by fengyoung(fengyang3@staff.weibo.com) 	
		int sbroadcast(vector<string>& vtrResp, const char* sReq, const bool bClearResp = true)
		{
			if(bClearResp) 
				vtrResp.clear(); 
			initialize();
			MapSplitIpPort map_ip_port;
			uint32_t key_num = ip_num_; 
			uint64_t n_keys[key_num];
			for(uint32_t index = 0; index < key_num; index++)
				n_keys[index] = index; 
			...
			for(MapSplitIpPort::iterator it = map_ip_port.begin(); it != map_ip_port.end(); it ++){
				...
				int ret = binary_client_send(cli[total_i], sReq, strlen(sReq)+1, log_id, true);
