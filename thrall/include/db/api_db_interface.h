...
class ApiDbInterface : public DbInterface{
	...
	protected:
		...
		int get_data_by_curl(const char* url, const char* request_key, 
			...
			/* specify URL to get */ 
			char request_url[COMPRESS_NUM] = {'0'};
	...
	public :
		...
		int get_i(uint16_t type_id, uint64_t n_key, uint32_t n_item_key, char* &p_result,
            ...
            char req[COMPRESS_NUM] = {'0'};
            if(other_flag){
                snprintf(req, COMPRESS_NUM, "%s", other_key);
            }else{
                snprintf(req, COMPRESS_NUM, "%"PRIu64, n_key);
            }

            initialize();

            uint16_t api_index = 0;
            if(vec_str_ip_.size() == 0)
                return 0;
            else{
                 srand((unsigned)time(NULL));
                 api_index = rand() % vec_str_ip_.size();
            }


            //cout<<"vec_str_ip_.size="<<vec_str_ip_.size()<<endl;
            //cout<<"ip="<<vec_str_ip_[api_index]<<endl;

            struct timeval tv_begin, tv_end;
            gettimeofday(&tv_begin, NULL);

            get_data_by_curl(vec_str_ip_[api_index].c_str(), req, NULL, p_result);
            split_char = db_info_struct_.value_split_first_char_;
            second_split_char = db_info_struct_.value_split_second_char_;

            gettimeofday(&tv_end, NULL);
            int time_cost = (tv_end.tv_sec - tv_begin.tv_sec)*1000 + (tv_end.tv_usec - tv_begin.tv_usec)/1000;
            LOG_ERROR("API %s GET %d ms", vec_str_ip_[api_index].c_str(), time_cost);
